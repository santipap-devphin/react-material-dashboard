...
import {Grid , Button , TextField , InputLabel , MenuItem , FormControl , Select  
, Autocomplete , Chip , Typography , Alert , AlertTitle , Switch ,FormGroup , FormControlLabel} from '@mui/material';
...
const submitBlog = async(e) => {
    ...
    if(cate && title && editor){
          ...
          let objs = {cateid:cate , title:title , content:editor , imgblog:imgData , tag:tagInsert , status:chkSwitch}

          try {
            

                const response = await endpoint.post("/blogs" , JSON.stringify(objs) , {headers:{'Content-Type': 'application/json'}})
                if(response.data.code === 1){
                  ...
                  setTimeout(function() {navicate("/blog")}, 3500);
  ...
  return (<> 
            ...
            <Grid container sx={{m:1}}>
                <Grid item xs={4}>
                    <Link to={"/blog"} style={{textDecoration: "none"}}><Button variant="contained" color="primary"><ArrowBackIcon/> กลับ</Button></Link>
                </Grid>
             </Grid>
             {
                alertStatus ? 
